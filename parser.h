...
double nodeValue[30] = { 0.0 }, jacMat[30][30] = { 0.0 }, result[30] = { 0.0 }, minDert[30] = { 0.0 }, result1[30] = { 0.0 }, preU = 0.0, stepsize = 0.001, stoptime = 0.4;
int Vsoure[10][4] = { 0 };    /*
...
void Component::connect(int conNum, Node* nodeIn) {//该组件的接口与节点连接
	if (conNum == 0) {
		con0.node = nodeIn;
		con0.flag = SET;
	}
...
void Node::printNodal(ofstream& outFile, int datum, int lastnode) {
	Connections* conPtr;
	conPtr = conList;//该节点的连接器链表
	while (conPtr != NULL) { //检查是否有连接器连接电压源。若没有，打印方程
...
void Node::printMNA(ofstream& outFile, int datum, int lastnode) {
	...
	while (conPtr != NULL) {
		if (conPtr->comp->getType() == VSource) {
			print = 1;
			outFile << endl;
			outFile << "F(" << lastnode + conPtr->comp->getcompNum() << ")=";//有几个电压源，就加几个补偿方程
		}
		conPtr = conPtr->next;
	}
	if (print) {
