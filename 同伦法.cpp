...
//void NR(double jacMat[][30], double result[], double minDert[], int number, int& count, double accurateValue, int datum, int lastnode);
NodeHead nodeList;
CompHead compList;
...
int main() {
	...
	std::cout << "Output saved to file: " << outName << endl;
	...
	if (flag != 3) {
		std::cout << "请输入初始数据的个数：" << endl;
		cin >> number;
		std::cout << "请输入初始数据值:" << endl;
	...
	if (flag == 1) {
		...
		std::cout << "iteration number:" << "  " << count << endl;
		std::cout << endl;
		for (int i = 0; i < number; i++) {
			std::cout << "▲x(" << i + 1 << ") =    " << minDert[i] << endl;
		}
		std::cout << endl;
		std::cout << "the result:" << endl;
		for (int i = 0; i < number; i++) {
			std::cout << "x(" << i + 1 << ") =    " << nodeValue[i + 1] << endl;
		}
	}
	if (flag == 2) {//************************同伦法*********************************
		double stepsize;
	   //Leakage Vector
		double Gleak = 1e-3;
		double a[100] = { 0,0.5000,0.4799, 0.9047, 0.6099, 0.6177, 0.8594, 0.8055, 0.5767 };
		std::cout << "请输入步长：" << endl;
		cin >> stepsize;
        ...
        //构造H(x)
		for (int i = 1; i <= number; i++) {
			result[i] = lambda * result[i] + (1 - lambda) * Gleak * (nodeValue[i] - a[i]);
		}
		//构造H(x)的雅可比矩阵
		for (int i = 1; i <= number; i++) {
			for (int j = 1; j <= number; j++) {
				if (i == j) 
					...
					jacMat[i][j] = jacMat[i][j] * lambda;//求对xj的偏导，nodeValue[i]是xi，所以i!=j时，第二项略去
			}
		}
	...
	if (flag == 3) {//************************瞬态分析*********************************
		FILE* fp;
		fp = fopen("tran.txt", "w");
		double U = 0, R = 0, C = 0, E = 0;
		double h = stepsize;
		compPtr = compList.getComp(0);
		while (compPtr != NULL) {
			if (!strcmp(compPtr->getName(), "R1")) {
				R = compPtr->getVal();
			}
			if (!strcmp(compPtr->getName(), "C1")) {
				C = compPtr->getVal();
			}
			if (!strcmp(compPtr->getName(), "VCC")) {
				E = compPtr->getVal();
			}
			compPtr = compPtr->getNext();
		}
		printf("R=%f,C=%f,E=%f", R, C, E);
		for (double time = stepsize; time < stoptime + stepsize; time = time + stepsize) {
			U = h * E / (R * C) + (1 - h / (R * C)) * U;
			fprintf(fp, "时间:%5.3f         电压值:%8.6f\n", time, U);
		}
	}
...
*/
void solving_method(double jacMat[][30], double result[], double minDert[], int number, int& count, double accurateValue, int datum, int lastnode, int flag, double lambda) {
